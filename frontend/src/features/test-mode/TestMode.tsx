...
import { filterWordGroupsByLanguagesAndTags } from "./filters";
import TestEndScreen from "./TestEndScreen";
import TestItem from "./TestItem";
import TestSettings, { type TestModeState } from "./TestSettings";
...
const TestMode = () => {
	...
	const [testObject, setTestObject] = useState<TestModeState>({
		test: {
			questions: [],
			correctAnswers: [],
		},
	...
	const availableTags = [...new Set(wordgroups.flatMap((wg) => wg.tags))];
	...
	const onNextQuestion = (skipped: boolean, correct?: boolean) => {
		if (!skipped && !correct) {
			setTestObject((prev) => ({
				...prev,
	...
	const generateTest = () => {
		...
		const questionCount = Math.min(
			...
			testObject.totalQuestions,
		);

		if (questionCount < testObject.totalQuestions) {
		...
		const randomWordGroups = new Set<WordGroup>();
		while (randomWordGroups.size < questionCount) {
			const randomGroup =
				availableQuestions[
					Math.floor(Math.random() * availableQuestions.length)
				];
			if (randomGroup) {
				randomWordGroups.add(randomGroup);
			}
		}
		const chosenWordGroups = [...randomWordGroups];
		const questions = chosenWordGroups.flatMap((wg) => {
		...
		const correctAnswers = chosenWordGroups.flatMap((wg) => {
			const translation = wg.translations.find(
				(t) => t.languageName === testObject.selectedLanguages[1],
			);
			return translation ? [[translation.word, ...translation.synonyms]] : [];
		});
		setTestObject((prev) => ({
			...prev,
			test: {
