...
export const useAiWordGeneration = () => {
  ...
  const [generatedWords, setGeneratedWords] = useState<SelectableGeneratedWord[]>([]);
  ...
  const handleWordGenerationFormSubmit = async (
    ...
    try {
      ...
      const generated: SelectableGeneratedWord[] = words.map((item) => ({
        ...item,
        isSelected: true,
  ...
  const handleSaveWordsToDatabase = async () => {
    ...
    const tags = [form.topic.toLowerCase(), form.skillLevel.toLowerCase()].filter(
      (t) => t.trim() !== "",
    );
