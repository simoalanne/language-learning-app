...
export type MatchingGameSettingsState = {
	open: boolean;
	gameStartedAtLeastOnce: boolean;
...
type MatchingGameSettingsProps = {
	open: boolean;
	handleClose: () => void;
	settings: MatchingGameSettingsState;
	setSettings: Dispatch<SetStateAction<MatchingGameSettingsState>>;
	availablePairs: number;
	handleCancel: () => void;
	handleExitToApp: () => void;
	availablePairsWithoutFilters: number;
	pairsPerTag: number[];
};
					...
					>
						...
						<SelectLanguagePair
							...
							setSelectedLanguages={(newSelectedLanguages) =>
								setSettings((prevSettings) => ({
									...prevSettings,
									selectedLanguages:
										newSelectedLanguages as MatchingGameSettingsState["selectedLanguages"],
									confirmMatch:
						...
						{settings.availableTags.length > 0 && (
							<>
								...
								<Typography variant="body2">
									pair must have at least one of the selected tags to be
									included in the game. The number in parentheses indicates how
									many pairs are available with the tag.
								</Typography>
								<ChipSelect
									options={settings.availableTags}
									labels={settings.availableTags.map(
										(tag, i) => `${tag} (${pairsPerTag[i]})`,
									)}
									selectedOptions={settings.selectedTags}
						...
						{availablePairs > 1 && (
							<>
										...
										>
											<Box sx={{ width: "75%" }}>
												...
												<Typography variant="body2">
													...
													each match before the cards stay flipped. If you
													answer incorrectly, the cards will flip back and you
								...
								<Typography variant="body1" sx={{ fontWeight: "bold" }}>
									{availablePairs === 2
										? "Number of pairs 2"
										: `Select the number of pairs between 2 and ${availablePairs}`}
								</Typography>
								<Slider
